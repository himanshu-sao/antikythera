...
The coverage assessment at commit `ee63ea1` closed 6 branch gaps in the `ce3360f` slice (ConditionalAction undefined-queue + save-Union 422), but deliberately scoped only the *changed* regions. **These handler bodies were *not* audited or covered** — `coverage` isn't installed in the venv, so the assessment was a branch-level walk, not a line-percentage. Pick up as a dedicated slice (install `coverage` → quote a numeric % over `studio_graph_engine.py` + `studio_router.py` → fill the gaps):
...
- [ ] **SC-Q2 — Router endpoints beyond save/run/preview** (`api/studio_router.py`): `list`+`get`+`delete` graph endpoints (round-trip + 404 paths), `list_runs` + `get_run_log`, `/undefined-queue`, `/schedulable-graphs`, and the skill (`/skills`) + `/integrations/status` endpoints — none have direct tests beyond the save-Union suite. Also exercise the `cron_schedule`/`cron_enabled`/`undefined_queue_cap` fields on `GraphCreateRequest`/`GraphUpdateRequest`.
- [ ] **SC-Q3 — Numeric coverage baseline**: add `coverage` (as a dev dep) to the venv and run `./venv/bin/python -m pytest tests/test_studio_*.py --cov=api.execution.studio_graph_engine --cov=api.studio_router` to get a hard %. Per `antikythera-prefers-targeted-tests`: targeted run only — do **not** wire this into a full-suite CI gate.
