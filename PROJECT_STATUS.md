...
- [ ] **P2.4 Normalize `automation/skill/builder` routers** — `automation_router /propose` uses regex/keyword matching (no LLM call), `skill_router /brainstorm` returns hardcoded static templates (`"MOCK AI Logic"`), `builder_router /generate` delegates to `AIAdapter.analyze()` which IS already wired through `LLMClient` (#6 above). The first two need to be routed through `LLMClient.chat()` with proper system prompts. ~~**Depends on P2.6**~~ (P2.6 now DONE — `_chat_bob()` is available, so this is unblocked).
    - **Re-verified 2026-07-19** (still open, no committed progress): `api/automation_router.py:54` `/propose` still has the `# MOCK AI LOGIC for demonstration of the flow` comment and the regex/keyword branch logic — no `LLMClient.chat()` call. `api/skill_router.py:25` `/brainstorm` still has the `# MOCK AI Logic` comment with hardcoded few-shot string concatenation — `skill_router` does not even import `LLMClient`. `builder_router /generate` → `AIAdapter.analyze()` confirmed already wired (unchanged). ⚠️ **Uncommitted WIP**: working tree has a modified `api/automation_router.py` that adds `import json`, `import logging`, and `from agents.llm_client import LLMClient` to the import block — but the symbols are **unused** (the `/propose` body is unchanged). Current branch `feat/p3.8.4-restore-pipeline-filters`. Treat this as a half-started P2.4 attempt to resume, not a completed fix.
