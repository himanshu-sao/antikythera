...
- [ ] **SC-Q1 — Engine handler bodies** (`api/execution/studio_graph_engine.py`): coverage for `_execute_query_node` (list/vector dispatch already has 3 tests in `test_studio_query_dispatch.py` — extend for the `fetch_resource`→`fetch` legacy fallback + unknown-action fallback), `_execute_fan_out_node` (the per-item `loop_over` iteration, `child_loop_context` merge, and `iteration_output` shape — currently no direct test), `_execute_ai_transform_node` (SafeExecutor dispatch + `DependencyRequiredError`/`SecurityError`/`SafeExecutorError` branches), and `_evaluate_condition` (each `ConditionType` + the `ConditionLogic` AND/OR short-circuit). **Note:** `_evaluate_condition` is exercised transitively by the conditional tests, but its per-`ConditionType` branches are not asserted directly. Baseline misses (line ranges): `168-201, 212-235, 275-333, 347-381, 394-411, 459-482, 534, 544-608, 618-652, 705-724, 768-801, 810-852, 863, 912-957`.
  - [x] **SC-Q1a** — DONE 2026-07-24. Extended `tests/test_studio_query_dispatch.py` with 3 tests: sync (non-coroutine) list/vector action branch (`studio_graph_engine.py:510` `else method(**params)`), `{{var}}` param resolution via `_resolve_params` (loop_context-priority over state, dot-path nested-state read, absent-var→`"None"` fallback at `:852`), and the adapter-absent fallback (unregistered `node.adapter` → `adapter is None` skips the list branch → `fetch_resource`→`fetch` → `ValueError("Unsupported adapter: …")`, NOT a `KeyError`). Full studio suite now **33 passed** (was 30); engine **52% (193 miss)** / total **59% (275 miss)**, vs. SC-Q3 baseline 58% (279 miss). SC-Q1 stays `[ ]` until Q1b/Q1c/Q1d land.
  - [x] **SC-Q1b** — DONE 2026-07-24. NEW `tests/test_studio_fan_out.py` (11 tests): iterate N items via a `source_handle="loop"` edge to a child ConditionalAction, assert one child exec per item + `child_loop_context` merge + `child_results == [{iterator_var: item}, …]`; source-None → `[]`, non-list source → `[]`, empty list → `[]`, no loop-edge, `loop_stack` push-then-pop. Target lines 561-608. Studio suite now **44 passed**; engine **60% (162 miss)** / total **64% (244 miss)** vs. SC-Q3 baseline 58% (279 miss). SC-Q1 stays `[ ]` until Q1c/Q1d land.
  - [x] **SC-Q1c** — DONE 2026-07-24. NEW `tests/test_studio_ai_transform.py` (13 tests): inline-script success (`result = input`), skill-ref success (loads persisted Skill.script), skill-not-found → `ValueError`, no-script-no-skill → `ValueError`, `DependencyRequiredError` (`import pandas`), `SecurityError` (`import os`), `SafeExecutorError` (`result = 1/0`), syntax error → `SafeExecutorError`; `{"input","state","loop"}` context wiring (pins dict construction at studio_graph_engine.py:636-640). Target lines 610-652. Studio suite now **57 passed**; engine **65% (139 miss)** / total **67% (221 miss)**, vs. SC-Q3 baseline 58% (279 miss).
  - [x] **SC-Q1d** — DONE 2026-07-24. NEW `tests/test_studio_condition_eval.py` (29 tests): each `ConditionType` (EQUALS/CONTAINS/REGEX_MATCH/IN_LIST/EXISTS) match + miss branches, `ConditionLogic` AND/OR short-circuit + multi-condition flat compounds, `logic not in {AND,OR}` fallthrough → `False`, non-Condition/Logic arg → `False`, dot-path field resolution. Pure helpers (sync). Target lines 764-801. Studio suite now **86 passed**; engine **71% (117 miss)** / total **70% (199 miss)**, vs. SC-Q3 baseline 58% (279 miss). **SC-Q1 complete** — all four sub-tasks landed.
- [x] **SC-Q2 — Router endpoints beyond save/run/preview** (`api/studio_router.py`): `list`+`get`+`delete` graph endpoints (round-trip + 404 paths), `list_runs` + `get_run_log`, `/undefined-queue`, `/schedulable-graphs`, and the skill (`/skills`) + `/integrations/status` endpoints — none have direct tests beyond the save-Union suite. Also exercise the `cron_schedule`/`cron_enabled`/`undefined_queue_cap` fields on `GraphCreateRequest`/`GraphUpdateRequest`. Baseline misses (line ranges): `64-78, 283-336, 352-371, 394-405, 421-422, 446-512, 521-553, 588-629`. **DONE 2026-07-24** — NEW `tests/test_studio_router_endpoints.py` (21 tests) covers all 13 router endpoints. Studio suite now **107 passed**; coverage delta tracked against SC-Q3 baseline.
- [x] **SC-Q3 — Numeric coverage baseline**: DONE 2026-07-23. Installed `coverage==7.15.2` + `pytest-cov==7.1.0` in the venv (pinned in `requirements.txt` under a "Dev/test tooling" block) and ran `./venv/bin/python -m pytest tests/test_studio_*.py --cov=api.execution.studio_graph_engine --cov=api.studio_router --cov-report=term-missing` → **30 passed, 0 failed**; **engine 51% (399 stmts / 197 miss), router 70% (269 / 82 miss), total 58% (668 / 279 miss)**. The `--cov-report=term-missing` line-range hit-lists are attached to SC-Q1 (engine) and SC-Q2 (router) above so future sessions know exactly what to cover. Per `antikythera-prefers-targeted-tests`: targeted run only — **not** wired into a full-suite CI gate.
- [ ] **SC-Q4 — Run path wiring**: the Studio `run_graph` → `start_run` → `_finalize_run` happy path with a 2-node graph (covered by `test_studio_dry_run` but only the dry-run short-circuit branch); add a non-dry (real-write) run through a mocked adapter to assert `total_matched`/`total_undefined`/`total_escalated` roll up + a `GraphRunLog` is persisted.
