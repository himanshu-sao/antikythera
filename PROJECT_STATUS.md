...
- [ ] **P3.2 Produce one real `execution_report.md`** *(parent — do the sub-tasks below in order)* — of ~26 requirement dirs, no original real backlog idea has been carried past spec (16 stalled at spec-only; 4 reached REVIEW with one-line stub `review.md`; 3 produced a 5-artifact set but stub `execution_report.md`; 1 fixture `E2E-ITEM` reached DONE with no artifacts). Prove one idea end-to-end through the pipeline, verifying every artifact is non-stub.
  ...
  - [ ] **P3.2.6** **Live gate: prove the real (default) LLM path produces a non-stub report.** Originally scoped for ``ibm_bob``; revised 2026-07-14 because the default provider was switched to ``google_gemma``/``gemma-4-31b-it`` (bob burns the user's tokens and is slow). ``google-genai`` SDK installed (v1.47.0) — the codebase imports ``google.genai`` which the old ``google-generativeai`` didn't supply. Live run (2026-07-14) outcome: **partial — not green**. Gemma (31b) succeeds on straightforward tool-calls (``write_file``, ``terminal``, ``read_file``) — each ~2–4s, 200-OK, real JSON — but (a) the planner **over-decomposes** a trivial idea into 11 tasks (vs the 3 the stubbed harness used), (b) Gemma returns empty/unparseable responses on meta/verification tasks (“Ensure /health handler returns…”, “Verify no db/auth/LLM imports…”) → ``json.loads`` fails → 5 retries → task incomplete, (c) on editorial tasks Gemma loops ``read_file`` instead of ``patch``/``write_file``. The run timed out at 5 min mid-loop on the last verification task. Net: the live pipeline *partially* works — Gemma drives real tool-calls and writes real files — but the multi-turn executor format stresses the model on meta/verification tasks. Next: consider a narrower prompt (fewer tasks per run, more explicit tool-usage constraints), try a stronger model (gemini-pro), or further tighten the planner checklist size. The ``google-genai`` dependency should be pinned (e.g. ``requirements.txt`` / ``pyproject.toml``) since it's now a runtime requirement of the default provider path.
