...
- [ ] **P3.3 Zombie RUNNING runs** — `workflow_runs.json` has 2 `RUNNING` runs open since 2026-05-24 (6 BLOCKED, 1 COMPLETED of 9). Reap/recover on startup. **Scope (2026-07-13):** (a) read `automation-ideas/workflow_runs.json` to confirm current state; (b) add a startup recovery step in `api/main.py` lifespan (or `api/managers/run_manager.py`) that marks any `RUNNING` run older than a threshold (e.g. 1 hour) as `FAILED` with a reason like `"orphaned: server restart"`; (c) add a test that writes a stale RUNNING run, calls the recovery function, and asserts it's reaped. Key files: `api/main.py` (lifespan), `api/managers/run_manager.py` (run state), `tests/test_run_manager.py` or similar.
...
- [ ] **P3.7 Empty workflow templates** — `github_pr_release` and `audit_test_tpl` in `workflow_templates.json` have `steps: []`. Either fill or remove. **Scope (2026-07-13):** read `automation-ideas/workflow_templates.json` to confirm; check if either template is referenced anywhere (UI dropdowns, trigger routes, tests); if unused, delete them; if referenced, fill with reasonable step definitions matching the template name's intent. Key files: `automation-ideas/workflow_templates.json`, `ui/src/components/WorkflowManager.tsx` (template picker), `api/workflow_router.py` (trigger endpoint).
