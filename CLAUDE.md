...
10. **`ibm_bob` is a CLI-based LLM provider**: Unlike the other 7 providers (all HTTP/OpenAI-compatible), `ibm_bob` shells out to the local `bob` binary (v1.0.6), which manages its own auth (browser SSO, 24h cache). Do not wire `ibm_bob` through the HTTP OpenAI client. Routed via `LLMClient._chat_bob()` in `agents/llm_client.py` (see P2.6 in `PROJECT_STATUS.md`, now DONE). The verified `bob` invocation: `bob --chat-mode ask --allowed-mcp-server-names "" --hide-intermediary-output -o text [-m <model>] <prompt-positional>` — do NOT use the deprecated `-p`, do NOT fabricate a model id (an unrecognized id crashes the binary with "[object Object]"). There are two distinct "BOB" things in this project: `ibm_bob` the LLM provider (`LLMClient`) and `bob_shell` the integration adapter (`api/adapters/bob_shell.py` / `ExecutionEngine`). They shell out to the same binary but serve different purposes.
