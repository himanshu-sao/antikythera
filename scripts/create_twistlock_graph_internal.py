#!/usr/bin/env python3
"""Create the Twistlock vulnerability triage graph using internal adapter for Slice-1 acceptance test."""
...
manager.delete_graph("graph_twistlock_vulnerability_triage")

# Create nodes using internal_adapter which works with local pipeline state
# Note: internal_adapter.list_items returns {"items": [...]}, so FanOut needs source="tickets.items"
query_node = QueryNode(
    node_id="query_twistlock",
    name="Query Twistlock Tickets",
    archetype=NodeArchetype.QUERY,
    description="Fetch items from local pipeline state (simulating Jira tickets)",
    adapter="internal_adapter",
    action="list_items",
    params={"stage": "REVIEW_SPEC"},
    output_ref="tickets",
)
...
fanout_node = FanOutNode(
    ...
    loop_over={"source": "tickets.items", "iterator_var": "ticket"},
)
...
cond_node = ConditionalActionNode(
    ...
    description="If severity is Critical, mark as escalated",
    ...
    true_action="internal_adapter",
    true_action_config={"stage": "EXECUTING"},
    true_output_ref="escalated_ticket_id",
    false_action=None,
    false_action_config={},
    false_output_ref=None,
    signature="If ticket severity == Critical, transition to EXECUTING",
)
...
graph = StudioGraph(
    ...
    description="Query items from local state, fan-out per item, AI-transform to extract fields, conditional action to escalate Critical items",
    version="1.0.0",
